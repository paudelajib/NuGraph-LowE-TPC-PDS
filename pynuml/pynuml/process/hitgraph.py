...
# ---------------------------------------------------------------------------
# DUNE FD-HD optical-detector geometry: channel -> (x, y, z) in cm.
#
# Extracted from opdet_geometry_raw.txt lines of the form
#   "Channel N => C:c O:o at (x,y,z) cm"
# 480 channels arranged on a 20 (y) x 24 (z) grid:
#   channels   0-239 sit at x = +0.05 cm (y > 0)
#   channels 240-479 sit at x = -0.05 cm (y < 0)
#
# Defined once at module scope: these are static detector geometry, so
# rebuilding them per event (as the previous inline definition did) was
# both wasteful and left them out of scope for the ophit nodes.
# ---------------------------------------------------------------------------
...
OPDET_POS_Z = torch.tensor([1357.47, 1357.47, 1357.47, 1357.47, 1357.47, 1357.47, 1357.47, 1357.47, 1357.47, 1357.47,
                            ...
                            83.9188, 83.9188, 83.9188, 83.9188, 83.9188, 83.9188, 83.9188, 83.9188, 83.9188, 83.9188,
                            35.1188, 35.1188, 35.1188, 35.1188, 35.1188, 35.1188, 35.1188, 35.1188, 35.1188, 35.1188])
OPDET_POS_X = torch.tensor([0.05] * 240 + [-0.05] * 240)

N_OPDET_CHANNELS = OPDET_POS_Y.numel()
assert OPDET_POS_Z.numel() == N_OPDET_CHANNELS == OPDET_POS_X.numel(), (
    "OpDet geometry tables disagree in length"
)


def _opdet_pos(channels) -> torch.Tensor:
    """Look up (x, y, z) in cm for an array of OpDet channel numbers.

    Fails loudly on an out-of-range channel rather than silently returning a
    plausible-but-wrong position, which is how the -999 wire_pos problem went
    unnoticed across a full production.
    """
    idx = torch.as_tensor(channels, dtype=torch.long)
    if idx.numel() and (int(idx.min()) < 0 or int(idx.max()) >= N_OPDET_CHANNELS):
        raise IndexError(
            f"OpDet channel out of range: got [{int(idx.min())}, {int(idx.max())}] "
            f"but the geometry table covers 0-{N_OPDET_CHANNELS - 1}. "
            "This sample is probably a different detector geometry."
        )
...
class HitGraphProducer(ProcessorBase):
    ...
    def __call__(self, evt: 'pynuml.io.Event') -> tuple[str, Any]:
        ...
        if self.optical:
            ...
            # node position
            # OpHit position: resolve hit_channel through the OpDet geometry
            # table rather than reading wire_pos.  wire_pos holds *wire numbers*
            # (NearWire projections), not a position, and is filled with the
            # -999 sentinel whenever HDF5Maker could not load its
            # OpDetGeometryFile.  Using the same table as data["pmt"].pos keeps
            # an OpHit and its PMT at identical coordinates by construction.
            data["ophit"].pos = _opdet_pos(ophits["hit_channel"].values)
            ...
            if "pos_y" in sum_pe.columns:
                ...
                # fall back to the module-level DUNE FD-HD geometry table
